      ...
      <RenderMobile>
        <div className="min-h-screen w-full bg-slate-50 text-slate-900 dark:bg-neutral-950 dark:text-white py-4">
          ...
          <div className="px-4">
            <div className="flex items-center justify-between gap-2 rounded-lg border border-slate-200 bg-white px-2 py-1 text-slate-900 dark:border-white/10 dark:bg-neutral-900 dark:text-white">
              <span className="min-w-0 flex-1 whitespace-normal break-words text-[11px] font-medium">
                Xin chào! Chào mừng đến với ATQ
              </span>
          ...
          >
            <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-gradient-to-r from-slate-100 to-slate-50 dark:border-white/10 dark:from-slate-700/70 dark:via-slate-600/60 dark:to-slate-500/50">
              ...
              <div className="relative p-3">
                {/* Row 1: headline + Withdraw */}
                <div className="grid grid-cols-3 items-center gap-2">
                  <div className="col-span-2">
                    <div className="text-[12px] font-medium text-slate-800 dark:text-white/90">
                      Bạn chưa đăng nhập, vui lòng đăng nhập trước
                    </div>
                  </div>
                  <button className="justify-self-end flex h-16 w-16 flex-col items-center justify-center gap-0.5 rounded-xl border border-white/30 bg-white/70 text-[10px] text-slate-700 shadow-sm hover:bg-white/80 dark:border-white/20 dark:bg-white/10 dark:text-white/90 dark:hover:bg-white/15">
                    <div className="flex h-7 w-7 items-center justify-center rounded-full bg-slate-200 text-slate-700 dark:bg-white/10 dark:text-white">
                      {/* withdraw icon */}
                      <svg
                        viewBox="0 0 24 24"
                        width="12"
          ...
          <motion.div
            ...
            className="px-4"
          >
            <div className="mt-6 text-center text-[11px] tracking-[0.2em] text-slate-500 dark:text-white/70">
              MENU NHANH
            </div>
            <motion.ul
              initial="hidden"
            ...
            >
              {quickItems.map((item) => (
                <motion.li
                  key={item.label}
                  variants={fadeItem}
                  className="flex flex-col items-center justify-center gap-2 rounded-xl bg-white p-3 border border-slate-200 dark:bg-white/5 dark:border-white/10"
                >
                  <div className="flex h-10 w-10 items-center justify-center rounded-full bg-brand/90 shadow-inner">
                    {/* simple glyph */}
                    <svg
                      viewBox="0 0 24 24"
                      width="18"
                  ...
                  <span className="text-[11px] text-center leading-tight text-slate-800 dark:text-white/90">
                    {item.label}
                  </span>
                </motion.li>
              ))}
          ...
          {/* Info list (simple list, not accordion) */}
          <motion.div
            initial={{ opacity: 0, y: 8 }}
            animate={{ opacity: 1, y: 0 }}
            ...
            className="px-4"
          >
            <div className="mt-6 rounded-2xl bg-white border border-slate-200 overflow-hidden dark:bg-white/5 dark:border-white/10">
              <motion.ul
                initial="hidden"
                animate="show"
                variants={listStagger}
              ...
              >
                {infoLinks.map((item) => (
                  <motion.li key={item.label} variants={fadeItem}>
                    <Link
                      href={item.href}
                      className="flex items-center justify-between px-4 py-3 active:bg-slate-100 dark:active:bg-white/5"
                    >
                      <div className="flex items-center gap-3">
                        <div className="flex h-9 w-9 items-center justify-center rounded-full bg-slate-100 border border-slate-200 text-slate-700 dark:bg-neutral-800 dark:border-white/10 dark:text-white">
                          {item.icon === "download" ? (
                            // download icon
                            <svg
                              viewBox="0 0 24 24"
...
const tabs = [
  ...
  { label: "Tài khoản", href: "/dang-ky", active: true },
];

const infoLinks = [
  { label: "TẢI ỨNG DỤNG", href: "#", icon: "download" },
  { label: "VỀ CHÚNG TÔI", href: "#", icon: "about" },
  { label: "CHÍNH SÁCH BẢO MẬT", href: "#", icon: "privacy" },
] as const;
